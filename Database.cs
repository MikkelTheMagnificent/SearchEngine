...
public void DisplayWordOccurrences()
{
    var selectCmd = _connection.CreateCommand();
    selectCmd.CommandText = @"
        SELECT w.id, w.name, COUNT(o.wordId) AS occurrence_count
        FROM word w
    ...
    using (var reader = selectCmd.ExecuteReader())
    {
        while (reader.Read())
        {
            ...
            var occurrenceCount = reader.GetInt32(2);
            Console.WriteLine($"<{wordName}, {wordId}> - {occurrenceCount}");
        }
    }
}
